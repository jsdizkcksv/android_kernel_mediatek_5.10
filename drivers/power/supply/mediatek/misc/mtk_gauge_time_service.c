...
static struct timespec64 gtimer_suspend_time;
...
void gtimer_start(struct gtimer *timer, int sec)
{
	...
	timer->endtime = timespec64_add(time_now, time);
	...
	list_for_each(pos, phead) {
		ptr = container_of(pos, struct gtimer, list);
		if (timespec64_compare(&timer->endtime, &ptr->endtime) < 0)
			break;
	}
	...
	if (list_empty(pos) != true) {
		ptr = container_of(pos, struct gtimer, list);
		if (timespec64_compare(&ptr->endtime, &time_now) < 0)
			wakeup = true;
		else {
			time = timespec64_sub(ptr->endtime, time_now);
			if (time.tv_sec < 1)
				time_interval = 1;
			else
...
static void gtimer_handler(void)
{
	...
	for (pos = phead->next; pos != phead;) {
		...
		ptr->endtime.tv_sec, ptr->interval,
		timespec64_compare(&time, &ptr->endtime));

		if (timespec64_compare(&time, &ptr->endtime) >= 0) {
			ptmp = pos;
			pos = pos->next;
			list_del_init(ptmp);
	...
	if (list_empty(pos) != true) {
		...
		time = timespec64_sub(ptr->endtime, time);
		if (time.tv_sec < 1)
			time_interval = 1;
		else
			time_interval = time.tv_sec;
...
static int gtimer_thread(void *arg)
{
	...
	while (1) {
		...
		duraction = timespec64_sub(endtime, stime);
...
static void gtimer_resume(void)
{
	...
	if (list_empty(pos) != true) {
		...
		if (timespec64_compare(&time, &ptr->endtime) >= 0) {
			...
			diff = timespec64_sub(ptr->endtime, time);
			if (diff.tv_sec < 1)
				time_interval = 1;
			else
	...
	struct notifier_block *notifier, unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE:	/* Going to suspend the system */
		ktime_get_boottime_ts64(&gtimer_suspend_time);
	...
	int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	if (list_empty(pos) != true) {
		...
		duraction = timespec64_sub(ptr->endtime, gtimer_suspend_time);
