...
static bool is_psy_voter_available(struct charger_manager *info)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_pe40_intf mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_pe40_intf mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!info->usb_icl_votable) {
		...
		if (!info->usb_icl_votable) {
			pr_err("mtk_pe40_intf Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
int mtk_pe40_set_mivr(struct charger_manager *pinfo, int uV)
{
	...
	if (pinfo->battery_67w)
		...
		ret = charger_dev_set_mivr(pinfo->chg5_dev, uV);
	if (ret < 0)
	...
	int adapter_mv, int adapter_ma, int ma)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_get_input_current(pinfo->chg5_dev, &oldmA);
	...
	if (pinfo->data.parallel_vbus && (oldmA * 2 > ma)) {
		if (chg2_enable) {
			/*charger_dev_set_input_current(pinfo->chg5_dev,
				ma * 1000 / 2);
			charger_dev_set_input_current(pinfo->chg2_dev,
				ma * 1000 / 2);*/
			vote(pinfo->usb_icl_votable, USBIN_PE40_INFT_1ST_REQUEST_VOTER,
					true, ma * 1000 / 2);
		} else {
			//charger_dev_set_input_current(pinfo->chg5_dev,
			//	ma * 1000);
		...
		//charger_dev_set_input_current(pinfo->chg5_dev, ma * 1000);
		vote(pinfo->usb_icl_votable, USBIN_PE40_INFT_1ST_REQUEST_VOTER,
					true, ma * 1000);
	}
	...
	int *adapter_vbus, int *adapter_ibus, int ma)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_get_mivr(pinfo->chg5_dev, &oldmivr);
	...
	if (pinfo->data.parallel_vbus && (oldmA * 2 > ma)) {
		if (chg2_enable) {
			...
			charger_dev_set_input_current(pinfo->chg2_dev,
				ma * 1000 / 2);*/
			vote(pinfo->usb_icl_votable, USBIN_PE40_INFT_PD_REQUEST_VOTER,
					true, ma * 1000 / 2);
		} else {
			//charger_dev_set_input_current(pinfo->chg5_dev,
			//	ma * 1000);
...
void mtk_pe40_reset(struct charger_manager *pinfo, bool enable)
{
	...
	if (pe40->is_connect == true) {
		...
		if (pinfo->battery_67w)
			...
			charger_dev_enable_termination(pinfo->chg5_dev, true);
	}
...
int mtk_pe40_get_ibus(struct charger_manager *pinfo, u32 *ibus)
{
	...
	if (pinfo->data.parallel_vbus) {
		if (pinfo->battery_67w)
			...
			ret = charger_dev_get_ibus(pinfo->chg5_dev, &chg1_ibus);

		if (is_enable) {
			if (pinfo->battery_67w)
				ret = charger_dev_get_ibat(pinfo->chg1_dev, &chg1_ibat);
			else
				ret = charger_dev_get_ibat(pinfo->chg5_dev, &chg1_ibat);
			if (ret < 0)
		...
		*ibus = chg1_ibus + chg2_ibus;
		...
		if (pinfo->battery_67w)
			...
			ret = charger_dev_get_ibus(pinfo->chg5_dev, ibus);
	}
...
int mtk_pe40_init_state(struct charger_manager *pinfo)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_enable_termination(pinfo->chg5_dev, false);
	...
	if (pinfo->battery_67w)
		...
		charger_dev_enable_powerpath(pinfo->chg5_dev, false);
	if (is_dual_charger_supported(pinfo) == true) {
		charger_dev_is_chip_enabled(pinfo->chg2_dev,
					&chg2_chip_enabled);
		if (chg2_chip_enabled) {
	...
	if (pinfo->battery_67w)
		...
		charger_dev_enable_powerpath(pinfo->chg5_dev, true);
	if (is_dual_charger_supported(pinfo) == true) {
		charger_dev_is_chip_enabled(pinfo->chg2_dev,
					&chg2_chip_enabled);
...
int mtk_pe40_cc_state(struct charger_manager *pinfo)
{
	...
	if (pinfo->battery_67w) {
		...
		charger_dev_get_mivr_state(pinfo->chg5_dev, &chg1_mivr);
		charger_dev_get_mivr(pinfo->chg5_dev, &mivr1);
	}
	...
	if (pinfo->battery_67w)
		...
		ccl = pinfo->chg5_data.charging_current_limit / 1000;
	...
	if (pinfo->battery_67w)
		...
		icl = pinfo->chg5_data.input_current_limit / 1000 *
					(100 - pinfo->data.ibus_err) / 100;
	...
	if (pinfo->data.parallel_vbus) {
		if (pinfo->battery_67w)
			...
			charger_dev_get_ibus(pinfo->chg5_dev, &compare_ibus);
	...
	if (pinfo->data.parallel_vbus) {
		if (((!pinfo->battery_67w && pinfo->chg5_data.thermal_input_current_limit != -1) ||
			(pinfo->battery_67w && pinfo->chg1_data.thermal_input_current_limit != -1)) ||
