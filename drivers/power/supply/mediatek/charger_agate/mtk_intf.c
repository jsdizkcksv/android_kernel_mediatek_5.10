...
int charger_is_chip_enabled(bool *en)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_is_chip_enabled(pinfo->chg5_dev, en);
}
...
int charger_enable_chip(bool en)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_enable_chip(pinfo->chg5_dev, en);
}
...
int charger_is_enabled(bool *en)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_is_enabled(pinfo->chg5_dev, en);
}
...
int charger_get_mivr_state(bool *in_loop)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_get_mivr_state(pinfo->chg5_dev, in_loop);
}
...
int charger_get_mivr(u32 *uV)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_get_mivr(pinfo->chg5_dev, uV);
}
...
int charger_set_mivr(u32 uV)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_set_mivr(pinfo->chg5_dev, uV);
}
...
int charger_get_input_current(u32 *uA)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_get_input_current(pinfo->chg5_dev, uA);
}
...
int charger_set_input_current(u32 uA)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_set_input_current(pinfo->chg5_dev, uA);
}
...
int charger_set_charging_current(u32 uA)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_set_charging_current(pinfo->chg5_dev, uA);
}
...
int charger_get_ibus(u32 *ibus)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_get_ibus(pinfo->chg5_dev, ibus);
}
...
int charger_set_constant_voltage(u32 uV)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_set_constant_voltage(pinfo->chg5_dev, uV);
}
...
int charger_enable_termination(bool en)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_enable_termination(pinfo->chg5_dev, en);
}
...
int charger_enable_powerpath(bool en)
{
	if (pinfo->battery_67w)
		...
		return charger_dev_enable_powerpath(pinfo->chg5_dev, en);
}
...
int adapter_set_cap_pd2_bq(int mV, int mA)
{
	...
	ret = adapter_dev_set_cap_bq(pinfo->pd_adapter, MTK_PD, mV, mA);
	if (ret == MTK_ADAPTER_REJECT)
		return ADAPTER_REJECT;
	else if (ret != 0)
		return ADAPTER_ERROR;
