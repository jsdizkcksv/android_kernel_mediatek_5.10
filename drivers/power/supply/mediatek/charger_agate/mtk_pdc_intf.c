...
static bool is_psy_voter_available(struct charger_manager *info)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_pdc_intf mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_pdc_intf mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!info->fcc_main_votable) {
		...
		if (!info->fcc_main_votable) {
			pr_err("mtk_pdc_intf Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
	...
	if (!info->usb_icl_votable) {
		...
		if (!info->usb_icl_votable) {
			pr_err("mtk_pdc_intf Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
int mtk_pdc_set_mivr(struct charger_manager *info, int uV)
{
	...
	if (info->battery_67w)
		...
		ret = charger_dev_set_mivr(info->chg5_dev, uV);
...
void mtk_pdc_check_cable_impedance(struct charger_manager *pinfo)
{
	...
	vote(pinfo->fcc_main_votable, FCCMAIN_PDC_INTF_AICR_VOTER, true, 2500000);
	mdelay(240);
	...
	vote(pinfo->usb_icl_votable, USBIN_PDC_INTF_AICR_VOTER, true, aicr_value);
	...
	if (pinfo->battery_67w)
		...
		ret = charger_dev_get_mivr_state(pinfo->chg5_dev, &mivr_state);
...
int mtk_pdc_get_max_watt(struct charger_manager *info)
{
	...
	if (info->pdc.pdc_max_watt_setting != -1)
		...
		if (info->battery_67w) {
			...
			if (info->chg5_data.thermal_charging_current_limit != -1)
				charging_current =
					info->chg5_data.thermal_charging_current_limit / 1000;
		}
...
int mtk_pdc_setup(struct charger_manager *info, int idx)
{
	...
	if (pd->pd_idx == idx) {
		if (info->battery_67w)
			...
			charger_dev_get_mivr(info->chg5_dev, &oldmivr);
	...
	if (pd->pd_idx != idx || force_update) {
		...
		if (info->battery_67w)
			...
			charger_dev_get_input_current(info->chg5_dev, &oldmA);
		oldmA = oldmA / 1000;
		...
		if (info->data.parallel_vbus && (oldmA * 2 > pd->cap.ma[idx])) {
			...
			charger_dev_set_input_current(info->chg2_dev,
					pd->cap.ma[idx] * 1000 / 2);*/
			vote(info->usb_icl_votable, USBIN_PDC_INTF_SETUP_VOTER, true,
					pd->cap.ma[idx] * 1000 / 2);
	...
	chr_err("[%s]idx:%d:%d:%d:%d vbus:%d cur:%d ret:%d pd_67w:%d\n", __func__,
		pd->pd_idx, idx, pd->pd_boost_idx, pd->pd_buck_idx,
		pd->cap.max_mv[idx], pd->cap.ma[idx], ret, info->battery_67w);
			...
			int *newidx)
{
	...
	if (info->battery_67w)
		...
		ret = charger_dev_get_ibus(info->chg5_dev, &ibus);
	if (ret < 0) {
	...
	if (info->data.parallel_vbus) {
		if (info->battery_67w)
			...
			ret = charger_dev_get_ibat(info->chg5_dev, &chg1_ibat);
		if (ret < 0)
	...
	if (info->battery_67w) {
		...
		charger_dev_get_mivr_state(info->chg5_dev, &chg1_mivr);
		charger_dev_get_mivr(info->chg5_dev, &mivr1);
	}
