...
static bool is_psy_voter_available(struct charger_manager *info)
{
	...
	if (!info->fcc_votable) {
		...
		if (!info->fcc_votable) {
			pr_err("mtk_charger Couldn't find FCC voltable\n");
			return false;
		}
	}
	...
	if (!info->fcc_main_votable) {
		...
		if (!info->fcc_main_votable) {
			pr_err("mtk_charger Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
	...
	if (!info->fv_votable) {
		...
		if (!info->fv_votable) {
			pr_err("mtk_charger Couldn't find FV voltable\n");
			return false;
		}
	}
	...
	if (!info->usb_icl_votable) {
		...
		if (!info->usb_icl_votable) {
			pr_err("mtk_charger Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
static bool is_psy_chg_voter_available(struct charger_manager *info)
{
	if (!info->input_suspend_votable) {
		info->input_suspend_votable = find_votable("INPUT_SUSPEND");
		if (!info->input_suspend_votable) {
			pr_err("mtk_charger Couldn't find INPUT_SUSPEND voltable\n");
			return false;
		}
	}
	...
	if (!info->chg_disable_votable) {
		...
		if (!info->chg_disable_votable) {
			pr_err("mtk_charger Couldn't find CHG_DISABLE voltable\n");
			return false;
		}
	}
	...
	int idx, int *input_current)
{
	...
	if (info != NULL) {
		...
		if (info->battery_67w)
			...
			charger_dev_get_input_current(info->chg5_dev, input_current);
		return 0;
	}
...
int charger_manager_get_ibus(int *ibus)
{
	...
	if (pinfo->battery_67w)
		charger_dev_get_ibus(pinfo->chg1_dev, ibus);
	else
		charger_dev_get_ibus(pinfo->chg5_dev, ibus);
...
int mtk_set_prop_input_suspend(int suspend)
{
	if (!is_psy_chg_voter_available(pinfo))
		return -EAGAIN;

	if (suspend) {
		pinfo->is_input_suspend = true;
		vote(pinfo->input_suspend_votable, CHG_USER_VOTER, true, 0);
	} else {
		pinfo->is_input_suspend = false;
		vote(pinfo->input_suspend_votable, CHG_USER_VOTER, false, 0);
	}
...
int mtk_get_prop_input_suspend(void)
{
	if (pinfo == NULL)
		return false;
...
static int charger_manager_set_input_current(int data)
{
	...
	if (!is_psy_voter_available(pinfo))
		return -EAGAIN;

	val.intval = data;

	vote(pinfo->usb_icl_votable, USBIN_THERMAL_INPUT_CURRENT_VOTER, true, val.intval);
...
int chg_set_fastcharge_mode(bool enable)
{
	...
	if (enable) {
		...
		if (pinfo->battery_67w)
			...
			pval.intval = 8960000;//pinfo->data.ffc_cv;

		rc = power_supply_set_property(pinfo->main_psy,
			POWER_SUPPLY_PROP_VOLTAGE_MAX, &pval);
		rc = vote(pinfo->fv_votable, FV_CHG_FASTCHARGER_MODE_VOTER, true, pval.intval);
		if (rc < 0)
	...
	//charger_dev_set_constant_voltage(pinfo->chg5_dev, pinfo->ffc_cv);
	vote(pinfo->fv_votable, FV_CHG_FASTCHARGER_MODE_VOTER, true, pinfo->ffc_cv);

	if (pinfo->battery_67w) {
		charger_dev_set_eoc_current(pinfo->chg1_dev, pinfo->ffc_ieoc);
	} else {
		charger_dev_set_eoc_current(pinfo->chg5_dev, pinfo->ffc_ieoc);
	}
...
void charger_manager_set_prop_system_temp_level(int temp_level)
{
	...
	pr_info("%s: charger_online:%d, temp_level:%d\n", __func__, charger_online, temp_level);
	if (pinfo == NULL || pinfo->usb_psy == NULL || !is_psy_voter_available(pinfo))
		return;
	...
	pr_info("%s, usb_type:%d hvdcp3_type:%d.\n", __func__, pinfo->usb_psy->desc->type, hvdcp3_type);
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS:
		thermal_fcc_ua = pinfo->thermal_mitigation_qc3p5[pinfo->system_temp_level];
		break;
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (pinfo->charge_120w_enable) {
			thermal_fcc_ua = pinfo->thermal_mitigation_qc3[pinfo->system_temp_level];
		} else {
			if (hvdcp3_type == HVDCP3_CLASSB_27W)
				thermal_fcc_ua = pinfo->thermal_mitigation_qc3_classb[pinfo->system_temp_level];
			else
				thermal_fcc_ua = pinfo->thermal_mitigation_qc3[pinfo->system_temp_level];
		}
		break;
	...
	pr_info("%s, system_temp_level:%d thermal_icl_ua:%d thermal_fcc_ua: %d.\n",
			 __func__, pinfo->system_temp_level, thermal_icl_ua, thermal_fcc_ua);

	if (pinfo->system_temp_level) {
		if (thermal_icl_ua) {
			vote(pinfo->usb_icl_votable, USBIN_THERMAL_INPUT_CURRENT_VOTER, true, thermal_icl_ua);
		} else {
			vote(pinfo->usb_icl_votable, USBIN_THERMAL_INPUT_CURRENT_VOTER, false, 0);
		}
		if (thermal_fcc_ua) {
			vote(pinfo->fcc_votable, FCC_CHG_THERMAL_VOTER, true, thermal_fcc_ua);
			vote(pinfo->fcc_main_votable, FCCMAIN_CHG_THERMAL_VOTER, true, thermal_fcc_ua);
		} else {
			vote(pinfo->fcc_votable, FCC_CHG_THERMAL_VOTER, false, 0);
			vote(pinfo->fcc_main_votable, FCCMAIN_CHG_THERMAL_VOTER, false, 0);
		}
	} else {
		vote(pinfo->usb_icl_votable, USBIN_THERMAL_INPUT_CURRENT_VOTER, false, 0);
		vote(pinfo->fcc_votable, FCC_CHG_THERMAL_VOTER, false, 0);
...
int charger_get_vbus(void)
{
	...
	if (pinfo->battery_67w)
		...
		ret = charger_dev_get_vbus(pinfo->chg5_dev, &vchr);
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (info->battery_temp >= info->data.temp_t4_thres) {
		...
		if ((sw_jeita->sm == TEMP_BELOW_T0)
			...
			if (batt_vol <= (info->data.jeita_temp_tn1_to_t0_cv_a - TAPER_CV_HY)) {
			...
			} else if (batt_vol > (info->data.jeita_temp_tn1_to_t0_cv_a - TAPER_CV_HY)
				&& batt_vol <= (info->data.jeita_temp_tn1_to_t0_cv_b - TAPER_CV_HY)) {
				jeita_current_limit = info->data.temp_tn1_to_t0_fcc_b;
				step_fcc = info->data.temp_tn1_to_t0_fcc_b;
	...
	if (info->data.enable_vote) {
		...
		ret = vote(info->fcc_votable, FCC_CHG_JEITA_VOTER, true, jeita_current_limit);
		if (ret > 0)
			pr_info("%s: set fcc jeita current limit success\n", __func__);

		ret = vote(info->fcc_main_votable, FCCMAIN_CHG_JEITA_VOTER, true, jeita_current_limit);
		if (ret > 0)
			pr_info("%s: set fcc_main jeita current limit success\n", __func__);
	}
	...
	if (sw_jeita->sm == TEMP_T2_TO_T3) {
		if (info->battery_temp >= EOC_TEMP && fastcharge_mode) {
			...
			if (info->battery_67w)
				...
				charger_dev_set_eoc_current(info->chg5_dev, info->data.ffc_ieoc_b);
			...
			if (info->battery_67w)
				...
				charger_dev_set_eoc_current(info->chg5_dev, info->data.ffc_ieoc);
		} else {
			pr_err("not ffc Set ieoc:%d\n", info->data.non_ffc_ieoc);
			if (info->battery_67w)
				charger_dev_set_eoc_current(info->chg1_dev, info->data.non_ffc_ieoc);
			else
				charger_dev_set_eoc_current(info->chg5_dev, info->data.non_ffc_ieoc);
		}
	}
...
int do_step_chg_state_machine(struct charger_manager *info)
{
	...
	pr_info("step_chg batt_vol:%d, step_fcc:%d.\n", batt_vol, step_fcc);

	if (info->data.enable_cv_step) {//cv_step
		if (batt_vol > (info->data.step_a - TAPER_CV_A_HY)
			&& batt_vol <= (info->data.step_b - TAPER_CV_A_HY)
			...
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, max(step_fcc, info->data.current_a));
			info->step_flag = STEP_A_TR;
			pr_info("step_chg cv step STEP_A_TR vbatt = %d , set fcc %d\n", batt_vol, step_fcc);
		}
		if (batt_vol > (info->data.step_b - TAPER_CV_A_HY) && step_fcc >= info->data.current_b) {
			step_fcc -= TAPER_STEP_MA;
			//charger_manager_set_current_limit(max(step_fcc, info->data.current_b), STEPCHG_FCC);
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, max(step_fcc, info->data.current_b));
		...
		if (batt_vol > (info->data.step_c - TAPER_CV_HY) && step_fcc >= info->data.current_c) {
			...
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, max(step_fcc, info->data.current_c));
			info->step_flag = STEP_C_TR;
			pr_info("step_chg cv step STEP_C_TR vbatt = %d , set fcc %d\n", batt_vol, step_fcc);
		}
		if (batt_vol > (info->data.step_d - TAPER_CV_HY) && step_fcc >= info->data.current_d) {
			step_fcc -= TAPER_STEP_MA;
			//charger_manager_set_current_limit(max(step_fcc, info->data.current_d), STEPCHG_FCC);
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, max(step_fcc, info->data.current_d));
			info->step_flag = STEP_D_TR;
			pr_info("step_chg cv step STEP_D_TR vbatt = %d , set fcc %d\n", batt_vol, step_fcc);
		}
	} else {//no cv_step
		if (batt_vol > info->data.step_a && info->step_flag != STEP_A_TR && info->step_flag != STEP_B_TR) {
			...
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_a);
			info->step_flag = STEP_A_TR;
		...
		if (batt_vol > info->data.step_b && info->step_flag != STEP_B_TR) {
			...
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_b);
			info->step_flag = STEP_B_TR;
			pr_info("step_chg STEP_B_TR vbatt = %d , set fcc %d\n", batt_vol, info->data.current_b);
		}
		if (batt_vol > info->data.step_c && info->step_flag != STEP_C_TR) {
			//charger_manager_set_current_limit(info->data.current_c, STEPCHG_FCC);
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_c);
			info->step_flag = STEP_C_TR;
	...
	case STEP_A_TR:
		if (batt_vol < info->data.step_a - info->data.step_hy_down_a) {
			...
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_max);
	...
	case STEP_C_TR:
		...
		break;
	case STEP_D_TR:
		if (batt_vol < info->data.step_d - info->data.step_hy_down_d) {
			info->step_flag = STEP_C_TR;
			//charger_manager_set_current_limit(info->data.current_c, STEPCHG_FCC);
			vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_c);
			pr_info("step_chg STEP_D_TR recover vbatt = %d\n", batt_vol);
		}
		break;
	default:
...
void mtk_charger_int_handler(void)
{
	...
	if ((mt_get_charger_type() == CHARGER_UNKNOWN && type_mode == POWER_SUPPLY_TYPEC_NONE) || (mt_get_charger_type() == CHARGER_UNKNOWN && type_mode == POWER_SUPPLY_TYPEC_SINK)) {
	...
	chr_err("wake_up_charger type_mode = %d\n",type_mode);
				...
				enum charger_type chr_type)
{
	...
	vote(info->usb_icl_votable, USBIN_CHG_PLUG_INOUT_VOTER, false, 0);
	vote(info->fcc_main_votable, FCCMAIN_CHG_PLUG_INOUT_VOTER, false, 0);
	vote(info->fcc_votable, FCC_CHG_THERMAL_VOTER, true, info->data.current_max);
	vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, true, info->data.current_max);
	charger_manager_set_prop_system_temp_level(info->system_temp_level);
...
static int mtk_charger_plug_out(struct charger_manager *info)
{
	...
	chr_err("%s\n", __func__);
	vote(info->fcc_votable, FCC_CHG_THERMAL_VOTER, false, 0);
	vote(info->fcc_votable, FCC_CHG_STEPCHG_VOTER, false, 0);
	vote(info->usb_icl_votable, USBIN_THERMAL_INPUT_CURRENT_VOTER, false, 0);
	...
	if (info->battery_67w) {
		...
		charger_dev_set_mivr(info->chg5_dev, info->data.min_charger_voltage);
	}
	//charger_dev_set_input_current(info->chg5_dev, 100000);
	vote(info->usb_icl_votable, USBIN_CHG_PLUG_INOUT_VOTER, true, 100000);
	//charger_dev_set_charging_current(info->chg5_dev, 0);
	vote(info->fcc_main_votable, FCCMAIN_CHG_PLUG_INOUT_VOTER, true, 0);
	if (is_client_vote_enabled_locked(info->usb_icl_votable, USBIN_PM_SM_VOTER))
		vote(info->usb_icl_votable, USBIN_PM_SM_VOTER, false, 0);
	if (is_client_vote_enabled_locked(info->fcc_main_votable, FCCMAIN_FG_CHARGE_FULL_VOTER))
		vote(info->fcc_main_votable, FCCMAIN_FG_CHARGE_FULL_VOTER, false, 0);
	if (is_client_vote_enabled_locked(info->fcc_votable, FCC_PM_FC2_CHARGE_ALGO_VOTER))
		vote(info->fcc_votable, FCC_PM_FC2_CHARGE_ALGO_VOTER, false, 0);
		...
		if (!mtk_pe40_get_is_connect(info) &&
			...
			if (vbat <
				...
				if (info->battery_67w)
					...
					charger_dev_set_mivr(info->chg5_dev,
						info->data.min_charger_voltage_2);
			} else if (vbat <
...
static void mtk_charging_related_information(struct charger_manager *info)
{
	union power_supply_propval val;
	...
	if (!is_psy_chg_voter_available(info))
		return;

	chr_err("%s: ICL vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->usb_icl_votable),
				get_effective_client_locked(info->usb_icl_votable));
	chr_err("%s: FCC vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->fcc_votable),
				get_effective_client_locked(info->fcc_votable));
	chr_err("%s: FV vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->fv_votable),
				get_effective_client_locked(info->fv_votable));
	chr_err("%s: FCC_MAIN vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->fcc_main_votable),
				get_effective_client_locked(info->fcc_main_votable));
	chr_err("%s: CHG_DISABLE vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->chg_disable_votable),
				get_effective_client_locked(info->chg_disable_votable));
	chr_err("%s: INPUT_SUSPEND vote value is %d voted by %s\n", __func__,
				get_effective_result_locked(info->input_suspend_votable),
				get_effective_client_locked(info->input_suspend_votable));
	...
	chr_err("%s: Usbin adc current[%d], Usbin adc voltage[%d]\n",
				__func__, usb_cur_in, usb_vol_in);

	chr_err("%s: Type-C orientation[%d], Type-C mode[%d], Real Charger Type[%d]\n",
				__func__, typec_orientation, typec_mode, charger_type);
}
...
static void charger_check_status(struct charger_manager *info)
{
	...
	mtk_charging_related_information(info);
				...
				struct device *dev)
{
	...
	info->battery_67w = of_property_read_bool(np, "battery_67w");
	...
	if (info->battery_67w)
		...
		info->chg5_data.charging_current_limit = info->data.ac_charger_current;
	...
	if (of_property_read_u32(np, "jeita_temp_above_t4_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_ABOVE_T4_CV:%d\n",
					JEITA_TEMP_ABOVE_T4_CV_LOW);
			info->data.jeita_temp_above_t4_cv = JEITA_TEMP_ABOVE_T4_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_ABOVE_T4_CV:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_T3_TO_T4_CV_LOW:%d\n",
					JEITA_TEMP_T3_TO_T4_CV_LOW);
			info->data.jeita_temp_t3_to_t4_cv = JEITA_TEMP_T3_TO_T4_CV_LOW;
		} else {
	...
	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_T2_TO_T3_CV:%d\n",
					JEITA_TEMP_T2_TO_T3_CV_LOW);
			info->data.jeita_temp_t2_to_t3_cv = JEITA_TEMP_T2_TO_T3_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_T2_TO_T3_CV:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_t1p5_to_t2_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_T1P5_TO_T2_CV:%d\n",
					JEITA_TEMP_T1P5_TO_T2_CV_LOW);
			info->data.jeita_temp_t1p5_to_t2_cv = JEITA_TEMP_T1P5_TO_T2_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_T1P5_TO_T2_CV:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_t1_to_t1p5_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_T1_TO_T1P5_CV:%d\n",
					JEITA_TEMP_T1_TO_T1P5_CV_LOW);
			info->data.jeita_temp_t1_to_t1p5_cv = JEITA_TEMP_T1_TO_T1P5_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_T1_TO_T1P5_CV:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_T0_TO_T1_CV:%d\n",
					JEITA_TEMP_T0_TO_T1_CV_LOW);
			info->data.jeita_temp_t0_to_t1_cv = JEITA_TEMP_T0_TO_T1_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_T0_TO_T1_CV:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_tn1_to_t0_cv_a", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_TN1_TO_T0_CV_A:%d\n",
					JEITA_TEMP_TN1_TO_T0_CV_A_LOW);
			info->data.jeita_temp_tn1_to_t0_cv_a = JEITA_TEMP_TN1_TO_T0_CV_A_LOW;
		} else {
			chr_err("use default JEITA_TEMP_TN1_TO_T0_CV_A:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_tn1_to_t0_cv_b", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_TN1_TO_T0_CV_B:%d\n",
					JEITA_TEMP_TN1_TO_T0_CV_B_LOW);
			info->data.jeita_temp_tn1_to_t0_cv_b = JEITA_TEMP_TN1_TO_T0_CV_B_LOW;
		} else {
			chr_err("use default JEITA_TEMP_TN1_TO_T0_CV_B:%d\n",
	...
	if (of_property_read_u32(np, "jeita_temp_below_t0_cv", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default JEITA_TEMP_BELOW_T0_CV:%d\n",
					JEITA_TEMP_BELOW_T0_CV_LOW);
			info->data.jeita_temp_below_t0_cv = JEITA_TEMP_BELOW_T0_CV_LOW;
		} else {
			chr_err("use default JEITA_TEMP_BELOW_T0_CV:%d\n",
	...
	if (of_property_read_u32(np, "temp_t3_to_t4_fcc", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_T3_TO_T4_FCC:%d\n",
					TEMP_T3_TO_T4_FCC_LOW);
			info->data.temp_t3_to_t4_fcc = TEMP_T3_TO_T4_FCC_LOW;
		} else {
			chr_err("use default TEMP_T3_TO_T4_FCC:%d\n",
					TEMP_T3_TO_T4_FCC);
	...
	if (of_property_read_u32(np, "temp_t2_to_t3_fcc", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_T2_TO_T3_FCC:%d\n",
					TEMP_T2_TO_T3_FCC_LOW);
			info->data.temp_t2_to_t3_fcc = TEMP_T2_TO_T3_FCC_LOW;
		} else {
			chr_err("use default TEMP_T2_TO_T3_FCC:%d\n",
					TEMP_T2_TO_T3_FCC);
	...
	if (of_property_read_u32(np, "temp_t1p5_to_t2_fcc", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_T1P5_TO_T2_FCC:%d\n",
					TEMP_T1P5_TO_T2_FCC_LOW);
			info->data.temp_t1p5_to_t2_fcc = TEMP_T1P5_TO_T2_FCC_LOW;
		} else {
			chr_err("use default TEMP_T1P5_TO_T2_FCC:%d\n",
					TEMP_T1P5_TO_T2_FCC);
	...
	if (of_property_read_u32(np, "temp_t1_to_t1p5_fcc", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_T1_TO_T1P5_FCC:%d\n",
					TEMP_T1_TO_T1P5_FCC_LOW);
			info->data.temp_t1_to_t1p5_fcc = TEMP_T1_TO_T1P5_FCC_LOW;
		} else {
			chr_err("use default TEMP_T1_TO_T1P5_FCC:%d\n",
					TEMP_T1_TO_T1P5_FCC);
	...
	if (of_property_read_u32(np, "temp_t0_to_t1_fcc", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_T0_TO_T1_FCC:%d\n",
					TEMP_T0_TO_T1_FCC_LOW);
			info->data.temp_t0_to_t1_fcc = TEMP_T0_TO_T1_FCC_LOW;
		} else {
			chr_err("use default TEMP_T0_TO_T1_FCC:%d\n",
					TEMP_T0_TO_T1_FCC);
	...
	if (of_property_read_u32(np, "temp_tn1_to_t0_fcc_a", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_TN1_TO_T0_FCC_A:%d\n",
					TEMP_TN1_TO_T0_FCC_A_LOW);
			info->data.temp_tn1_to_t0_fcc_a = TEMP_TN1_TO_T0_FCC_A_LOW;
		} else {
			chr_err("use default TEMP_TN1_TO_T0_FCC_A:%d\n",
	...
	if (of_property_read_u32(np, "temp_tn1_to_t0_fcc_b", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default TEMP_TN1_TO_T0_FCC_B:%d\n",
					TEMP_TN1_TO_T0_FCC_B_LOW);
			info->data.temp_tn1_to_t0_fcc_b = TEMP_TN1_TO_T0_FCC_B_LOW;
		} else {
			chr_err("use default TEMP_TN1_TO_T0_FCC_B:%d\n",
	...
	if (of_property_read_u32(np, "bif_threshold1", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default BIF_THRESHOLD1:%d\n",
					BIF_THRESHOLD1_LOW);
			info->data.bif_threshold1 = BIF_THRESHOLD1_LOW;
		} else {
			chr_err("use default BIF_THRESHOLD1:%d\n",
					BIF_THRESHOLD1);
	...
	if (of_property_read_u32(np, "bif_threshold2", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default BIF_THRESHOLD2:%d\n",
					BIF_THRESHOLD2_LOW);
			info->data.bif_threshold2 = BIF_THRESHOLD2_LOW;
		} else {
			chr_err("use default BIF_THRESHOLD2:%d\n",
					BIF_THRESHOLD2);
	...
	if (of_property_read_u32(np, "bif_cv_under_threshold2", &val) >= 0)
		...
		if (info->battery_67w) {
			chr_err("use default BIF_CV_UNDER_THRESHOLD2:%d\n",
					BIF_CV_UNDER_THRESHOLD2_LOW);
			info->data.bif_cv_under_threshold2 = BIF_CV_UNDER_THRESHOLD2_LOW;
		} else {
			chr_err("use default BIF_CV_UNDER_THRESHOLD2:%d\n",
	...
	if (info->data.enable_vote) {
		...
		if (of_property_read_u32(np, "step_d", &val) >= 0)
			info->data.step_d = val;
		else {
			if (info->battery_67w) {
				chr_err("use default STEP_D:%d\n", STEP_D_LOW);
				info->data.step_d = STEP_D_LOW;
			} else {
				chr_err("use default STEP_D:%d\n", STEP_D);
				info->data.step_d = STEP_D;
			}
		}
		...
		if (of_property_read_u32(np, "current_a", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.current_a = CURRENT_A_LOW;
			} else {
				chr_err("use default CURRENT_A:%d\n", CURRENT_A);
		...
		if (of_property_read_u32(np, "current_b", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.current_b = CURRENT_B_LOW;
			} else {
				chr_err("use default CURRENT_B:%d\n", CURRENT_B);
		...
		if (of_property_read_u32(np, "current_c", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.current_c = CURRENT_C_LOW;
			} else {
				chr_err("use default CURRENT_C:%d\n", CURRENT_C);
		...
		if (of_property_read_u32(np, "current_d", &val) >= 0)
			info->data.current_d = val;
		else {
			if (info->battery_67w) {
				chr_err("use default CURRENT_D:%d\n", CURRENT_D_LOW);
				info->data.current_d = CURRENT_D_LOW;
			} else {
				chr_err("use default CURRENT_D:%d\n", CURRENT_D);
				info->data.current_d = CURRENT_D;
			}
		}
		...
		if (of_property_read_u32(np, "current_max", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.current_max = CURRENT_MAX_LOW;
			} else {
				chr_err("use default CURRENT_MAX:%d\n", CURRENT_MAX);
		...
		if (of_property_read_u32(np, "step_hy_down_d", &val) >= 0)
			info->data.step_hy_down_d = val;
		else {
			chr_err("use default STEP_HY_DOWN_D:%d\n", STEP_HY_DOWN_D);
			info->data.step_hy_down_d = STEP_HY_DOWN_D;
		}
	}
	...
	if (info->data.enable_ffc) {
		if (of_property_read_u32(np, "non_ffc_ieoc", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.non_ffc_ieoc = NON_FFC_IEOC_LOW;
			} else {
				chr_err("use default NON_FFC_IEOC:%d\n", NON_FFC_IEOC);
		...
		if (of_property_read_u32(np, "non_ffc_cv", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.non_ffc_cv = NON_FFC_CV_LOW;
			} else {
				chr_err("use default NON_FFC_CV:%d\n", NON_FFC_CV);
		...
		if (of_property_read_u32(np, "ffc_ieoc", &val) >= 0)
			...
			if (info->battery_67w) {
				...
				info->data.ffc_ieoc = FFC_IEOC_LOW;
			} else {
				chr_err("use default FFC_IEOC:%d\n", FFC_IEOC);
		...
		if (of_property_read_u32(np, "ffc_ieoc_b", &val) >= 0)
			...
			if (info->battery_67w) {
				chr_err("use default FFC_IEOC:%d\n", FFC_IEOC_B_LOW);
				info->data.ffc_ieoc_b = FFC_IEOC_B_LOW;
			} else {
				chr_err("use default FFC_IEOC:%d\n", FFC_IEOC_B);
				info->data.ffc_ieoc_b = FFC_IEOC_B;
			}
		}
	...
	info->charge_120w_enable = of_property_read_bool(np, "charge-120w-enable");
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (pinfo->battery_67w) {
		...
		pr_debug("[Battery] %s : %x\n",
				__func__, pinfo->chg5_data.thermal_input_current_limit);
		return sprintf(buf, "%u\n",
				pinfo->chg5_data.thermal_input_current_limit);
	}
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (pinfo->battery_67w)
			...
			pinfo->chg5_data.thermal_input_current_limit = reg;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (pinfo->battery_67w) {
		...
		pr_debug("[Battery] %s : %x\n",
				__func__, pinfo->chg5_data.thermal_charging_current_limit);
		return sprintf(buf, "%u\n",
				pinfo->chg5_data.thermal_charging_current_limit);
	}
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		...
		if (pinfo->battery_67w) {
			...
			pinfo->chg5_data.thermal_charging_current_limit = reg;
			pr_debug("[Battery] %s: %x\n", __func__,
					pinfo->chg5_data.thermal_charging_current_limit);
		}
	}
...
static int mtk_chg_en_power_path_show(struct seq_file *m, void *data)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_is_powerpath_enabled(pinfo->chg5_dev, &power_path_en);
		...
		const char *buffer, size_t count, loff_t *data)
{
	...
	if (ret == 0) {
		if (info->battery_67w)
			...
			charger_dev_enable_powerpath(info->chg5_dev, enable);
...
static int mtk_chg_en_safety_timer_show(struct seq_file *m, void *data)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_is_safety_timer_enabled(pinfo->chg5_dev, &safety_timer_en);
	...
	const char *buffer, size_t count, loff_t *data)
{
	...
	if (ret == 0) {
		if (info->battery_67w)
			...
			charger_dev_enable_safety_timer(info->chg5_dev, enable);
...
void sc_update(struct charger_manager *pinfo)
{
	...
	if (pinfo->battery_67w)
		...
		charger_dev_get_charging_current(pinfo->chg5_dev, &pinfo->sc.data.data[SC_IBAT_SETTING]);
	...
	if (pinfo->battery_67w)
		...
		charger_dev_get_ibus(pinfo->chg5_dev, &pinfo->sc.data.data[SC_IBUS]);
...
static int mtk_chg_disable_vote_callback(struct votable *votable, void *data,
			int chg_disable, const char *client)
{
	struct charger_manager *chg = data;

	if (chg->charge_120w_enable) {
		charger_dev_enable(chg->chg5_dev, !chg_disable);
	} else {
		charger_dev_enable(chg->chg1_dev, !chg_disable);
	}

	return 0;
}

static int mtk_input_suspend_vote_callback(struct votable *votable, void *data,
			int suspend, const char *client)
{
	...
	if (suspend && !in_suspend) {
		charger_dev_set_hiz_mode(chg->chg5_dev, suspend);
		vote(chg->chg_disable_votable, CHG_INPUT_SUSPEND_VOTER, suspend, 0);
		in_suspend = true;
...
static int mtk_charger_create_votable(struct charger_manager *chg)
{
	int rc = 0;

	if (!mtk_batt_create_votable) {
		rc = -EINVAL;
		chr_err("Couldn't mtk_batt_create_votable rc=%d\n", rc);
		return rc;
	}

	if (mtk_batt_destroy_votable) {
		rc = -EINVAL;
		chr_err("Couldn't mtk_batt_destroy_votable rc=%d\n", rc);
		return rc;
	}
	...
	if (chg->fcc_votable == NULL) {
		rc = -EINVAL;
		chr_err("Couldn't find FCC votable rc=%d\n", rc);
		return rc;
	}
	...
	if (chg->fcc_main_votable == NULL) {
		rc = -EINVAL;
		chr_err("Couldn't find FCC Main votable rc=%d\n", rc);
		return rc;
	}
	...
	if (chg->fv_votable == NULL) {
		rc = -EINVAL;
		chr_err("Couldn't find FV votable rc=%d\n", rc);
		return rc;
	}
	...
	if (chg->usb_icl_votable == NULL) {
		rc = -EINVAL;
		chr_err("Couldn't find USB_ICL votable rc=%d\n", rc);
		return rc;
	}

	chg->chg_disable_votable = create_votable("CHG_DISABLE", VOTE_SET_ANY,
					mtk_chg_disable_vote_callback,
					chg);
...
destroy_votable:
	...
	destroy_votable(chg->input_suspend_votable);
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->chg5_data.input_current_limit_by_aicl = -1;
	...
	info->chg1_data.input_current_limit = 100000;
	info->chg5_data.input_current_limit = 100000;
	...
	ret = mtk_batt_init(&info->chg_param);
	if (ret < 0) {
		chr_err("Couldn't init mtk_batt_init rc=%d\n",
			ret);
		return ret;
	}
	...
	if (ret < 0) {
		chr_err("Couldn't init mtk_charger_create_votable rc=%d\n",
			ret);
		return ret;
	}
	...
	ret = vote(info->fv_votable, FV_CHG_FASTCHARGER_MODE_VOTER,
				true, info->data.non_ffc_cv);
	if (ret < 0)
		pr_err("Couldn't write non ffc voltage max:%d\n", ret);
	...
	if (info->chg5_dev != NULL && info->do_event != NULL) {
		info->chg5_nb.notifier_call = info->do_event;
		register_charger_device_notifier(info->chg5_dev,
						&info->chg5_nb);
		charger_dev_set_drvdata(info->chg5_dev, info);
	}
	...
	if (info->battery_67w)
		...
		charger_dev_enable_safety_timer(info->chg5_dev, false);
#endif
