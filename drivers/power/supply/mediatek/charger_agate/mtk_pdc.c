...
static bool is_psy_voter_available(struct charger_manager *info)
{
	if (!mtk_batt_create_votable) {
		pr_err("mtk_pdc mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("mtk_pdc mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!info->fcc_main_votable) {
		...
		if (!info->fcc_main_votable) {
			pr_err("mtk_pdc Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
	...
	if (!info->fv_votable) {
		...
		if (!info->fv_votable) {
			pr_err("mtk_pdc Couldn't find FV voltable\n");
			return false;
		}
	}
	...
	if (!info->usb_icl_votable) {
		...
		if (!info->usb_icl_votable) {
			pr_err("mtk_pdc Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
void disable_pdc_voter(void)
{
	if (!is_psy_voter_available(votable_info))
		return;

	vote(votable_info->usb_icl_votable, USBIN_PDC_SETUP_VOTER, false, 0);
	vote(votable_info->fv_votable, FV_PDC_SET_CV_VOTER, false, 0);
}
...
int pdc_setup(int idx)
{
	...
	if (pd->pd_idx != idx || force_update) {
		...
		if (oldmA > pd->cap.ma[idx]) {
			...
			vote(votable_info->usb_icl_votable, USBIN_PDC_SETUP_VOTER, true,
						pd->cap.ma[idx] * 1000);
		}
...
int pdc_check_leave(void)
{
	...
	if (max_mv * ibus <= PD_MIN_WATT || mivr_state) {
		if (mivr_state)
...
int pdc_set_current(void)
{
	...
	vote(votable_info->usb_icl_votable, USBIN_PDC_SETCURRENT_VOTER, true,
						pd->data.input_current_limit);
	//charger_set_charging_current(pd->data.charging_current_limit);
	vote(votable_info->fcc_main_votable, FCCMAIN_PDC_SETCURRENT_VOTER, true,
				pd->data.charging_current_limit);
...
int pdc_set_cv(void)
{
	...
	if (battery_psy) {
		power_supply_get_property(battery_psy,
			POWER_SUPPLY_PROP_LOW_POWER_CHARGING, &val);
		if (val.intval)
			pd->data.battery_cv = 4480000;
		else
			pd->data.battery_cv = 8960000;
	} else {
		pd->data.battery_cv = 8960000;
	}

	//charger_set_constant_voltage(pd->data.battery_cv);
	vote(votable_info->fv_votable, FV_PDC_SET_CV_VOTER, true,
						pd->data.battery_cv);
#else
	if (!is_psy_voter_available(votable_info))
		return -EAGAIN;
...
int pdc_run(void)
{
	...
	if (battery_psy) {
		...
		val.intval = get_effective_result_locked(votable_info->fcc_votable);
		if (ret) {
