...
#include "../mediatek/charger_agate/mtk_charger_intf.h"
...
static const char *chg_name = "quinary_chg";
...
static bool is_psy_voter_available(struct bq25790 *bq)
{
	if (!mtk_batt_create_votable) {
		pr_err("bq25790_charger mtk_batt_create_votable\n");
		return false;
	}

	if (mtk_batt_destroy_votable) {
		pr_err("bq25790_charger mtk_batt_destroy_votable\n");
		return false;
	}
	...
	if (!bq->fcc_main_votable) {
		...
		if (!bq->fcc_main_votable) {
			bq_dbg(PR_OEM, "bq25790_charger Couldn't find FCC_MAIN voltable\n");
			return false;
		}
	}
	...
	if (!bq->fv_votable) {
		...
		if (!bq->fv_votable) {
			bq_dbg(PR_OEM, "bq25790_charger Couldn't find FV voltable\n");
			return false;
		}
	}

	if (!bq->usb_icl_votable) {
		bq->usb_icl_votable = find_votable("USB_ICL");
		if (!bq->usb_icl_votable) {
			bq_dbg(PR_OEM, "bq25790_charger Couldn't find USB_ICL voltable\n");
			return false;
		}
	}
...
static int bq25790_tune_vbus_volt(bool up, int pulse)
{
	...
	ret = charger_dev_tune_hvdcp_dpdm(chg2_dev, val);

	bq_dbg(PR_OEM, "tune adapter voltage %s %s\n", up ? "up" : "down",
			ret ? "fail" : "successfully");

	return ret;
}

static int bq25790_raise_qc_vbus(struct bq25790 *bq, int num)
{
	int tune_vbus_retry = 0;

	bq_dbg(PR_OEM, "bq25790_raise_qc_vbus start.\n");

	while (bq->vbus_volt < 9000000) {
		bq25790_tune_vbus_volt(VOLT_UP, num);
		msleep(20);
		bq25790_update_status(bq);
		tune_vbus_retry++;

		bq_dbg(PR_OEM, "vbus_volt:%d, tune_vbus_retry:%d.\n",
				bq->vbus_volt, tune_vbus_retry);

		if ((tune_vbus_retry > 230) && (bq->vbus_volt < 8400000)) {
			tune_vbus_retry = 0;
			pr_err("Failed to tune adapter volt into valid range\n");
			return false;
		}

		if (bq->vbus_volt < 4000000) {
			bq_dbg(PR_OEM, "Vbus is plug out.\n");
			return false;
		}
	}

	if (bq->vbus_volt > 9500000) {
		bq25790_tune_vbus_volt(VOLT_DOWN, 1);
		bq25790_update_status(bq);
		bq_dbg(PR_OEM, "DOWN:vbus_volt:%d.\n", bq->vbus_volt);
	}

	return true;
}

static int bq25790_set_charge_voltage(struct charger_device *chg_dev, u32 volt)
{
	...
	bq_dbg(PR_OEM, "bq25790_api bq25790_set_charge_voltage:%d\n", volt);
...
static int bq25790_set_hiz_mode(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;
...
static int bq25790_get_hiz_mode(struct charger_device *chg_dev, int *status)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;
...
static int bq25790_get_charge_current(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u16 reg_val;
...
static int bq25790_get_charge_voltage(struct charger_device *chg_dev, u32  *volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u16 reg_val;
...
static int bq25790_get_input_current_limit(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u16 reg_val;
...
static int bq25790_set_input_volt_limit(struct bq25790 *bq, int volt)
{
	...
	bq_dbg(PR_OEM, "bq25790_api bq25790_set_input_volt_limit:%d\n", volt);
...
static int bq25790_get_pfm_otg(struct bq25790 *bq, int *status)
{
	int ret;
	u8 reg_val;

	ret = bq25790_read_reg(bq, BQ25790_REG_CHG_CTRL3, &reg_val);

	if (!ret)
		*status = reg_val & BQ25790_PFM_OTG_DIS_MASK;

	return ret;
}

static int bq25790_set_pfm_otg(struct bq25790 *bq, bool enable)
{
	int ret;
	u8 reg_val;

	bq_dbg(PR_OEM, "bq25790_api set_pfm_otg : %d.\n", enable);

	if (enable == false)
		reg_val = BQ25790_PFM_OTG_DIS;
	else
		reg_val = BQ25790_PFM_OTG_EN;

	reg_val <<= BQ25790_PFM_OTG_DIS_SHIFT;

	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_CTRL3,
				BQ25790_PFM_OTG_DIS_MASK, reg_val);

	return ret;
}

static int bq25790_get_pfm_fwd(struct bq25790 *bq, int *status)
{
	int ret;
	u8 reg_val;
	...
	if (!ret)
		*status = reg_val & BQ25790_PFM_FWD_DIS_MASK;
...
static int bq25790_get_otg_ooa(struct bq25790 *bq, int *status)
{
	int ret;
	u8 reg_val;
	...
	if (!ret)
		*status = reg_val & BQ25790_DIS_OTG_OOA_MASK;

	return ret;
}

static int bq25790_set_otg_ooa(struct bq25790 *bq, bool enable)
{
	int ret;
	u8 reg_val;

	if (enable == false)
		reg_val = BQ25790_DIS_OTG_OOA;
	else
		reg_val = BQ25790_EN_OTG_OOA;

	reg_val <<= BQ25790_DIS_OTG_OOA_SHIFT;

	ret = bq25790_update_byte_bits(bq, BQ25790_REG_CHG_CTRL3,
				BQ25790_DIS_OTG_OOA_MASK, reg_val);

	return ret;
}

static int bq25790_get_fwd_ooa(struct bq25790 *bq, int *status)
{
	int ret;
	u8 reg_val;
	...
	if (!ret)
		*status = reg_val & BQ25790_DIS_FWD_OOA_MASK;
...
static int bq25790_set_term_enable(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;

	bq_dbg(PR_OEM, "bq25790_api bq25790_set_term_enable:%d\n", enable);
...
static int bq25790_set_term_current(struct charger_device *chg_dev, u32 curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;

	bq_dbg(PR_OEM, "bq25790_api bq25790_set_term_current:%d\n", curr);
	...
	bq_dbg(PR_OEM, "bq25790_api term_current reg_val:%d\n", reg_val);

	return ret;
}

static int bq25790_is_charging_done(struct charger_device *chg_dev, bool *done)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	bq_dbg(PR_OEM, "bq25790_api bq_charge_done %d\n", bq->charge_done);
...
static int bq25790_set_vrechg_voltage(struct charger_device *chg_dev, int vol)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret, vreg;
	u8 reg_val;
	...
	bq->rechg_vol = vol;
	vreg = get_effective_result_locked(bq->fv_votable);
	...
	bq_dbg(PR_OEM, "bq25790_api rechg_vol:%d, regval:%x\n", bq->rechg_vol, reg_val);
	return ret;
}
...
static int bq25790_set_wdt_timer(struct bq25790 *bq, int time)
{
	...
	bq_dbg(PR_OEM, "bq25790_api bq25790_set_wdt_timer:%02x\n", reg_val);
...
static int bq25790_enable_safety_timer(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;

	bq_dbg(PR_OEM, "bq25790_api bq25790_enable_safety_timer:%d\n", enable);
...
static int bq25790_is_safety_timer_enabled(
				struct charger_device *chg_dev, bool *en)
{
	...
	ret = bq25790_read_reg(bq, BQ25790_REG_TIMER_CTRL, &reg_val);
	if (ret) {
		return ret;
	}
	*en = !!(reg_val & BQ25790_SAFETY_TIMER_EN_MASK);
...
static int bq25790_charge_enable(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;

	bq_dbg(PR_OEM, "bq25790_api bq25790_charge_enable:%d\n", enable);
...
static int bq25790_read_bus_volt(struct charger_device *chg_dev, u32 *volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	int reg_val;
...
static int bq25790_read_bus_curr(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	int reg_val;
...
static int bq25790_read_bat_volt(struct charger_device *chg_dev, u32 *volt)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	int reg_val;
...
static int bq25790_read_bat_curr(struct charger_device *chg_dev, u32 *curr)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	s16 reg_val;
...
static int bq25790_get_min_ichg(struct charger_device *chg_dev, u32 *curr)
{
	*curr = 300000;
	return 0;
}

static int bq25790_get_min_aicr(struct charger_device *chg_dev, u32 *curr)
{
	*curr = 100000;
	return 0;
}

static int bq25790_set_mivr(struct charger_device *chg_dev, u32 uV)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	bq_dbg(PR_OEM, "bq25790_api bq25790_set_mivr:%d\n", uV);
	return bq25790_set_input_volt_limit(bq, uV);
}

static int bq25790_get_mivr(struct charger_device *chg_dev, u32 *uV)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_get_input_volt_limit(bq, uV);
}

static int bq25790_get_mivr_state(struct charger_device *chg_dev, bool *in_loop)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	u8 status;
	int ret;
	...
	*in_loop = !!(status & BQ25790_VINDPM_STAT_MASK);
	return 0;
}

static int bq25790_enable_power_path(struct charger_device *chg_dev,
					    bool en)
{
	bq_dbg(PR_OEM, "bq25790_api bq25790_enable_power_path:%d\n", en);

	return bq25790_charge_enable(chg_dev, en);
}

static int bq25790_is_power_path_enabled(struct charger_device *chg_dev,
						bool *en)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	u8 status;
	...
	ret = bq25790_read_reg(bq, BQ25790_REG_CHG_CTRL0, &status);
	if (ret) {
		return ret;
	}
	*en = !!(status & BQ25790_CHARGE_EN_MASK);

	return 0;
}

static int bq25790_get_adc(struct charger_device *chg_dev, enum adc_channel chan,
			  int *min, int *max)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int channel;
	...
	ret = bq25790_read_word(bq, channel, &reg_val);
	if (ret) {
		return ret;
	}
	*min = reg_val;
...
static int bq25790_kick_wdt(struct charger_device *chg_dev)
{
	struct bq25790 *bq = charger_get_data(chg_dev);

	return bq25790_reset_wdt(bq);
}

static int bq25790_enable_otg(struct charger_device *chg_dev, bool enable)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	u8 reg_val;

	ret = bq25790_set_wdt_timer(bq, enable ? 160 : false);
	if (ret < 0) {
		bq_dbg(PR_DEBUG, "%s: set wdt fail, en = %d\n", __func__, enable);
		return ret;
	}

	bq_dbg(PR_OEM, "bq25790_api bq25790_enable_otg:%d\n", enable);
	if (enable) {
		/* enable acdrv1 */
		bq25980_set_acdrv1_en(enable);
		/* set pfm otg */
		bq25790_set_pfm_otg(bq, enable);
...
static int bq25790_set_otg_current_limit(struct charger_device *chg_dev,
						u32 uA)
{
	struct bq25790 *bq = charger_get_data(chg_dev);
	int ret;
	...
	uA /= 1000;
	bq_dbg(PR_OEM, "bq25790_api bq25790_set_otg_current_limit:%d\n", uA);

	if (uA < BQ25790_IOTG_LIMIT_BASE)
		uA = BQ25790_IOTG_LIMIT_BASE;

	uA -= BQ25790_IOTG_LIMIT_BASE;
	reg_val = uA / BQ25790_IOTG_LIMIT_STEP;
	reg_val <<= BQ25790_IOTG_LIMIT_SHIFT;

	ret = bq25790_update_word_bits(bq, BQ25790_REG_IOTG_REGULATION,
				BQ25790_IOTG_LIMIT_MASK, reg_val);

	return ret;
}

static int bq25790_do_event(struct charger_device *chg_dev, u32 event,
				   u32 args)
{
	bq_dbg(PR_OEM, "bq25790_api bq25790_do_event:%d\n", event);
	switch (event) {
	case EVENT_EOC:
...
static const struct charger_ops bq25790_chg_ops = {
	/* enable */
	.enable = bq25790_charge_enable,
	/* charging current */
	.set_charging_current = bq25790_set_charge_current,
	.get_charging_current = bq25790_get_charge_current,
	.get_min_charging_current = bq25790_get_min_ichg,
	/* charging voltage */
	.set_constant_voltage = bq25790_set_charge_voltage,
	.get_constant_voltage = bq25790_get_charge_voltage,
	/* charging input current */
	.set_input_current = bq25790_set_input_curr_limit,
	.get_input_current = bq25790_get_input_current_limit,
	.get_min_input_current = bq25790_get_min_aicr,
	/* set termination current */
	.set_eoc_current = bq25790_set_term_current,
	.is_charging_done = bq25790_is_charging_done,
	/* charging mivr */
	.set_mivr = bq25790_set_mivr,
	.get_mivr = bq25790_get_mivr,
	.get_mivr_state = bq25790_get_mivr_state,
	/* charing termination */
	.enable_termination = bq25790_set_term_enable,
	/* Power path */
	.enable_powerpath = bq25790_enable_power_path,
	.is_powerpath_enabled = bq25790_is_power_path_enabled,
	/* safety timer */
	.enable_safety_timer = bq25790_enable_safety_timer,
	.is_safety_timer_enabled = bq25790_is_safety_timer_enabled,
	/* OTG */
	.enable_otg = bq25790_enable_otg,
	.set_boost_current_limit = bq25790_set_otg_current_limit,
	/* ADC */
	.get_adc = bq25790_get_adc,
	.get_vbus_adc = bq25790_read_bus_volt,
	.get_ibus_adc = bq25790_read_bus_curr,
	.get_ibat_adc = bq25790_read_bat_curr,
	/* kick wdt */
	.kick_wdt = bq25790_kick_wdt,
	/* event */
	.event = bq25790_do_event,
	/* hiz mode */
	.get_hiz_mode = bq25790_get_hiz_mode,
	.set_hiz_mode = bq25790_set_hiz_mode,
	/* recharge vbat */
	.set_recharge_vbat = bq25790_set_vrechg_voltage,
};
...
static int bq25790_get_prop_charge_status(struct bq25790 *bq)
{
	...
	bq_dbg(PR_OEM, "bq25790_api charge_state:%d, usb_present:%d\n",
				bq->charge_state, bq->usb_present);
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		bq25790_get_input_current_limit(bq->chg_dev, &val->intval);
	...
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION:
		...
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION: %d\n",
					val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_RECHARGE_VBAT:
		...
		break;
	case POWER_SUPPLY_PROP_PFM_OTG_DIS:
		bq25790_get_pfm_otg(bq, &val->intval);
		break;
	case POWER_SUPPLY_PROP_PFM_FWD_DIS:
		bq25790_get_pfm_fwd(bq, &val->intval);
		break;
	case POWER_SUPPLY_PROP_DIS_OTG_OOA:
		bq25790_get_otg_ooa(bq, &val->intval);
		break;
	case POWER_SUPPLY_PROP_DIS_FWD_OOA:
		bq25790_get_fwd_ooa(bq, &val->intval);
		break;
	default:
		return -EINVAL;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT: %d\n",
					val->intval);
		vote(bq->fcc_main_votable, BQ25790_PROP_VOTER,
	...
	case POWER_SUPPLY_PROP_HIZ_MODE:
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_HIZ_MODE: %d\n",
					val->intval);
		bq25790_set_hiz_mode(bq->chg_dev, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT: %d\n",
					val->intval);
		vote(bq->usb_icl_votable, BQ25790_PROP_VOTER,
				val->intval > 0 ? true : false, val->intval);
		break;
	case POWER_SUPPLY_PROP_ARTI_VBUS_ENABLE:
		...
		break;
	case POWER_SUPPLY_PROP_TERMINATION_CURRENT:
		bq25790_set_term_current(bq->chg_dev, val->intval);
		break;
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION:
	...
	case POWER_SUPPLY_PROP_FORCE_MAIN_ICL:
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_FORCE_MAIN_ICL: %d\n",
					val->intval);
		vote_override(bq->usb_icl_votable, BQ25790_PROP_VOTER,
				val->intval > 0 ? true : false, val->intval);
		break;
	case POWER_SUPPLY_PROP_RECHARGE_VBAT:
		bq_dbg(PR_DEBUG, "POWER_SUPPLY_PROP_RECHARGE_VBAT: %d\n",
					val->intval);
		bq25790_set_vrechg_voltage(bq->chg_dev, val->intval);
		break;
	case POWER_SUPPLY_PROP_PFM_OTG_DIS:
		bq25790_set_pfm_otg(bq, val->intval);
		break;
	case POWER_SUPPLY_PROP_PFM_FWD_DIS:
		bq25790_set_pfm_fwd(bq, val->intval);
		break;
	case POWER_SUPPLY_PROP_DIS_OTG_OOA:
		bq25790_set_otg_ooa(bq, val->intval);
		break;
	case POWER_SUPPLY_PROP_DIS_FWD_OOA:
		bq25790_set_fwd_ooa(bq, val->intval);
		break;
	default:
		return -EINVAL;
	}
...
static int bq25790_update_charging_profile(struct bq25790 *bq)
{
	...
	case POWER_SUPPLY_TYPE_USB_PD:
		bq25790_set_input_volt_limit(bq, BQ25790_PPS_VINDPM_MV);
		if (adapter_33w_flag) {
			iindpm = BQ25790_PPS_33W_IINDPM_MA;
			fcc = BQ25790_PPS_33W_FCC;
		} else {
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		...
		} else if (bq->hvdcp_class == HVDCP3_CLASSA_18W) {
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS:
		if (bq->hvdcp_class == HVDCP3_P_CLASSB_27W) {
			iindpm = BQ25790_HVDCP_P_B_IINDPM_MA;
			fcc = BQ25790_HVDCP_P_B_FCC;
			bq25790_set_input_volt_limit(bq, BQ25790_HVDCP_P_B_VINDPM_MV);
		} else if (bq->hvdcp_class == HVDCP3_P_CLASSA_18W) {
			iindpm = BQ25790_HVDCP_P_A_IINDPM_MA;
			fcc = BQ25790_HVDCP_P_A_FCC;
			bq25790_set_input_volt_limit(bq, BQ25790_HVDCP_P_A_VINDPM_MV);
		}
		break;
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		bq25790_set_input_volt_limit(bq, BQ25790_SDP_VINDPM_MV);
		iindpm = BQ25790_FLOAT_IINDPM_MA;
		fcc = BQ25790_FLOAT_FCC;
		break;
	...
	bq_dbg(PR_OEM, "%s: charge_type:%d, iindpm:%d, fcc:%d\n",
			__func__, bq->charger_type, iindpm, fcc);

	vote(bq->usb_icl_votable, BQ25790_INIT_VOTER, true, iindpm);
	vote(bq->fcc_main_votable, BQ25790_INIT_VOTER, true, fcc);

	rerun_election(bq->fcc_main_votable);
...
static int bq25790_init_device(struct bq25790 *bq)
{
	...
	ret = bq25790_set_term_current(bq->chg_dev, bq->platform_data->iterm);
	if (ret)
		bq_dbg(PR_OEM, "Failed to set termination current, ret = %d\n", ret);

	ret = bq25790_enable_safety_timer(bq->chg_dev, bq->platform_data->safe_timer_en);
	if (ret)
	...
	bq25790_set_term_enable(bq->chg_dev, false);
//	bq25790_set_pfm_otg(bq, true);
...
static void bq25790_update_status(struct bq25790 *bq)
{
	...
	bq25790_read_bus_volt(bq->chg_dev, &bq->vbus_volt);
	bq25790_read_bat_volt(bq->chg_dev, &bq->vbat_volt);
	bq25790_read_bus_curr(bq->chg_dev, &bq->ibus_curr);
	bq25790_read_bat_curr(bq->chg_dev, &bq->ichg_curr);
...
static void bq25790_fv_check(struct bq25790 *bq)
{
	...
	if ((status == POWER_SUPPLY_STATUS_CHARGING) && (vcell > fv_max)) {
		fv = vreg - 10000;
		vote(bq->fv_votable, FV_BQ25790_FULL_CHK_VOTER, true, fv);
		bq_dbg(PR_OEM, "vcell:%d is high, reduce fv:%d\n", vcell, fv);
	}

	bq_dbg(PR_OEM, "vcell:%d, fv:%d, fv_max:%d, fastcharge_mode:%d\n",
		vcell, vreg, fv_max, chg_get_fastcharge_mode());
}
...
static void bq25790_status_change_work(struct work_struct *work)
{
	...
	bool hvdcp_flag = false;
	int ret;

	bq_dbg(PR_OEM, "bq25790_status_change_work start\n");
	if (!bq->usb_psy)
		return;
	...
	bq_dbg(PR_OEM, "charger_type:%d\n", charger_type);

	if (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3 ||
			charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS) {
		ret = power_supply_get_property(bq->usb_psy,
	...
	bq_dbg(PR_OEM, "charger_type:%d, bq->charger_type:%d.\n", charger_type, bq->charger_type);
	bq_dbg(PR_OEM, "hvdcp_class:%d,  bq->hvdcp_class:%d.\n", hvdcp_class, bq->hvdcp_class);

	if ((charger_type != POWER_SUPPLY_TYPE_UNKNOWN)
			&& ((bq->charger_type != charger_type) || (bq->hvdcp_class != hvdcp_class)
			|| adapter_33w_flag)) {

		bq->charger_type = charger_type;

		if ((bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
				|| (bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
				|| (bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)) {
			...
			if (bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP) {
				...
				if ((bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
					...
					hvdcp_class = prop.intval;
					bq->charger_type = charger_type;
					bq_dbg(PR_OEM, "Update QC3 | QC3P5.\n");
				} else if ((bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
						&& (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)) {
					bq->charger_type = charger_type;
					hvdcp_flag = true;
					bq_dbg(PR_OEM, "Keep QC2.\n");
				} else {
					bq->charger_type = charger_type;
					bq_dbg(PR_OEM, "Other type:%d.\n", bq->charger_type);
					return;
				}
			}

			if ((bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)
					|| (bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)
					|| hvdcp_flag) {

				if (bq->charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3_PLUS)
					ret = bq25790_raise_qc_vbus(bq, 10);
				else
					ret = bq25790_raise_qc_vbus(bq, 1);

				bq->hvdcp_class = hvdcp_class;
				bq25790_update_status(bq);
				bq_dbg(PR_OEM, "vbus_volt:%d.\n", bq->vbus_volt);

				if (bq->vbus_volt < 8800000) {
					bq->charger_type = POWER_SUPPLY_TYPE_USB_HVDCP;
					bq->hvdcp_class = HVDCP3_NONE;
					bq_dbg(PR_OEM, "Raise vbus failed.\n");
				}
				bq25790_update_charging_profile(bq);
			}
		} else {
	...
	for (addr = 0x0; addr <= 0x48; addr++) {
		for (i = 0; i < 15; i++) {
			if (addr == bq25790_word_reg[i]) {
				...
				pr_err("bq25790_dump_reg: 0x%.2x=0x%.4x\n", addr, word_val);
				goto next;
			}
		}
		bq25790_read_reg(bq, addr, &val);
		pr_err("bq25790_dump_reg: Reg0x%.2x=0x%.2x\n", addr, val);
next:
		continue;
	}
}
...
static void bq25790_charge_monitor_workfunc(struct work_struct *work)
{
	...
	bq_dbg(PR_OEM, "bq25790_charge_monitor_workfunc\n");
	...
	schedule_delayed_work(&bq->charge_monitor_work, 5 * HZ);
}
...
static void bq25790_charge_irq_workfunc(struct work_struct *work)
{
	...
	bq_dbg(PR_OEM, "bq25790_charge_irq_workfunc start\n");
	if (!is_psy_voter_available(bq))
		return;
	...
	if (!bq->power_good) {
		...
		vote(bq->fv_votable, FV_BQ25790_FULL_CHK_VOTER, false, 0);
		vote(bq->usb_icl_votable, BQ25790_INIT_VOTER, false, 0);
		vote(bq->fcc_main_votable, BQ25790_INIT_VOTER, false, 0);
		...
		int fcc_ua, const char *client)
{
	...
	rc = bq25790_set_charge_current(bq->chg_dev, fcc_ua);
	if (rc < 0) {
		...
		int fv_uv, const char *client)
{
	...
	rc = bq25790_set_charge_voltage(bq->chg_dev, fv_uv);
	if (rc < 0) {
		bq_dbg(PR_OEM, "failed to allocate register map\n");
		return rc;
	}
		...
		int icl_ua, const char *client)
{
	...
	rc = bq25790_set_input_curr_limit(bq->chg_dev, icl_ua);
	if (rc < 0) {
		bq_dbg(PR_OEM, "failed to allocate register map\n");
		return rc;
	}
		...
		void *data, int disable, const char *client)
{
	...
	bq25790_charge_enable(bq->chg_dev, !disable);
...
static int bq25790_create_votable(struct bq25790 *bq)
{
	...
	*/
	...
	if (IS_ERR(bq->awake_votable)) {
		rc = PTR_ERR(bq->awake_votable);
		bq->awake_votable = NULL;
		goto destroy_votable;
	}
...
destroy_votable:
	...
	//destroy_votable(bq->arti_vbus_dis_votable);
	destroy_votable(bq->awake_votable);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	vote_override(bq->fcc_main_votable, BQ25790_MTBF_VOTER, bq->mtbf, BQ25790_CDP_FCC);
	vote_override(bq->usb_icl_votable, BQ25790_MTBF_VOTER, bq->mtbf, BQ25790_CDP_IINDPM_MA);
				 ...
				 const struct i2c_device_id *id)
{
	...
	bq->chg_dev = charger_device_register(chg_name, bq->dev,
			bq, &bq25790_chg_ops, &bq->chg_props);
	if (IS_ERR_OR_NULL(bq->chg_dev)) {
		bq_dbg(PR_OEM, "bq25790 charger device register fail\n");
		goto err_1;
	}
	...
	ret = bq25790_create_votable(bq);
	if (ret) {
		bq_dbg(PR_OEM, "Failed to bq25790_create_votable\n");
		return ret;
	}
	...
	schedule_delayed_work(&bq->charge_irq_work, 0);
	schedule_delayed_work(&bq->charge_status_change_work, 4 * HZ);
