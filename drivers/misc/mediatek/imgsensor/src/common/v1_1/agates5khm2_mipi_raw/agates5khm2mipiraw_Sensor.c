...
#include "agates5khm2mipiraw_Sensor.h"
#include "agates5khm2_seamless_switch.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = AGATES5KHM2_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[11] = {
	...
	{ 12000, 9000, 4000,  3000,   4000, 3000,  4000, 3000,	  0,	0,	4000, 3000,    0,	 0,  4000, 3000}, // custom5 in sensor zoom4x
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	S5KHM2_LOG_DBG("framerate = %d, min framelength =%d, should enable? \n", framerate, min_framelength_en);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	S5KHM2_LOG_DBG("set_gain %d \n", gain);
	if (gain < imgsensor_info.min_gain || gain > imgsensor_info.max_gain) {
		S5KHM2_LOG_DBG("Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	S5KHM2_LOG_DBG("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
{
	S5KHM2_LOG_DBG("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		mdelay(5);
		//write_cmos_sensor(0x6214, 0xE9E0); // close clock
		//write_cmos_sensor(0x6218, 0x0110); // close clock
...
#if MULTI_WRITE
#if 0 /*for debug*/
	S5KHM2_LOG_DBG("[agates5khm2-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d start", len);
	for (i = 0; i < len/2; i++)
		S5KHM2_LOG_DBG("[agates5khm2-debug]readback addr 0x%04x, 0x%02x,\n",
			para[2*i], read_cmos_sensor(para[2*i]) & 0xFF);
	S5KHM2_LOG_DBG("[agates5khm2-debug]read back from sensor ++++++++++++++++++++++++++++++++++ len: %d end", len);
#endif
#endif
	return 0;
}
...
/*static void read_cali_data_from_eeprom(void)
{
	...
	if (ret != 0) {
		S5KHM2_LOG_DBG("iReadRegI2C error");
		return;
	}
...
{
	if (Is_Read_Otp_Data == 1) {
		S5KHM2_LOG_DBG("write pdc to sensor E\n");
...
{
	if (Is_Read_Otp_Data == 1) {
		S5KHM2_LOG_DBG("write xtalk to sensor E\n");
...
static void sensor_init(void)
{
    ...
    mdelay(10);
    //write_cmos_sensor(0x6214, 0xE9F0);
    //write_cmos_sensor(0x6218, 0x0110);

    S5KHM2_LOG_DBG("write sensor init setting +\n");

    // SW page
	s5khm2_fast_write_cmos_sensor(agates5khm2_swpage_setting,
		sizeof(agates5khm2_swpage_setting) / sizeof(kal_uint16));

    // Global
	s5khm2_fast_write_cmos_sensor(agates5khm2_init_setting,
		sizeof(agates5khm2_init_setting) / sizeof(kal_uint16));

	// GOS
	s5khm2_fast_write_cmos_sensor(agates5khm2_gos_setting,
		sizeof(agates5khm2_gos_setting) / sizeof(kal_uint16));
...
{
	S5KHM2_LOG_DBG("write sensor previews setting\n");

	if (enable_seamless) {
		// Fast Mode Change
		s5khm2_fast_write_cmos_sensor(agates5khm2_fmc_setting,
			sizeof(agates5khm2_fmc_setting) / sizeof(kal_uint16));

		table_write_cmos_sensor(agates5khm2_fastmode_preview,
			sizeof(agates5khm2_fastmode_preview) / sizeof(kal_uint16));
	} else {
		table_write_cmos_sensor(agates5khm2_preview_setting,
			sizeof(agates5khm2_preview_setting) / sizeof(kal_uint16));
	}
}	/*	preview_setting  */
...
#if SEAMLESS_SWITCH_ENABLE
	table_write_cmos_sensor(agates5khm2_switch_normal,
		sizeof(agates5khm2_switch_normal) / sizeof(kal_uint16));
#endif

	table_write_cmos_sensor(agates5khm2_normal_video_setting,
		sizeof(agates5khm2_normal_video_setting) / sizeof(kal_uint16));
}
...
static void hs_video_setting(void)
{
	...
	table_write_cmos_sensor(agates5khm2_hs_video_setting,
		sizeof(agates5khm2_hs_video_setting) / sizeof(kal_uint16));
}
...
static void custom2_setting(void)
{
	...
	table_write_cmos_sensor(agates5khm2_custom2_setting,
		sizeof(agates5khm2_custom2_setting) / sizeof(kal_uint16));
}
...
static void custom3_setting(void)
{
	...
	table_write_cmos_sensor(agates5khm2_custom3_setting,
		sizeof(agates5khm2_custom3_setting) / sizeof(kal_uint16));
}
...
static void custom4_setting(void)
{
	...
	table_write_cmos_sensor(agates5khm2_custom4_setting,
		sizeof(agates5khm2_custom4_setting) / sizeof(kal_uint16));
}
...
#if SEAMLESS_SWITCH_ENABLE
	table_write_cmos_sensor(agates5khm2_switch_fastmode_2x,
		sizeof(agates5khm2_switch_fastmode_2x) / sizeof(kal_uint16));
#else
	table_write_cmos_sensor(agates5khm2_custom5_setting,
		sizeof(agates5khm2_custom5_setting) / sizeof(kal_uint16));
#endif
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	S5KHM2_LOG_DBG("AGATES5KHM2 get_imgsensor_id in\n");
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				S5KHM2_LOG_DBG("probe success, i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			} else {
				S5KHM2_LOG_DBG("Read sensor id fail,i2c write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			}
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	S5KHM2_LOG_DBG("PLATFORM:MT6893, MIPI 3LANE\n");
	//S5KHM2_LOG_DBG("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
	//sensor have two i2c address 0x6c 0x6d & 0x21 0x20, we should detect the module used i2c address
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
...
{
	S5KHM2_LOG_DBG("loading preview_setting\n");
...
static kal_uint32 capture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading capture_setting\n");
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 normal_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading normal_video_setting\n");
...
static kal_uint32 hs_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading hs_video_setting\n");
...
static kal_uint32 custom1(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom1_setting\n");
...
static kal_uint32 custom2(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom2_setting\n");
...
static kal_uint32 custom3(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom3_setting\n");
...
static kal_uint32 custom4(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom4_setting\n");
...
static kal_uint32 custom5(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom5_setting\n");
...
static kal_uint32 custom6(
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	S5KHM2_LOG_DBG("loading custom6_setting\n");
...
{
	S5KHM2_LOG_DBG("enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(enum MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		if (imgsensor.frame_length > imgsensor.shutter) {
			...
			/*No need to set*/
			S5KHM2_LOG_DBG("frame_length %d < shutter %d", imgsensor.frame_length, imgsensor.shutter);
		}
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
		if (framerate == 0)
	...
	kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM1:
		...
		if (gain != 0) {
			write_cmos_sensor(AGAIN_ADDR, gain / 2);
		}
		if (shutter != 0) {
			write_cmos_sensor(COARSE_INTEGRATION_TIME_ADDR, (shutter) & 0xFFFF);
		}

		table_write_cmos_sensor(agates5khm2_switch_fastmode_preview,
			sizeof(agates5khm2_switch_fastmode_preview) / sizeof(kal_uint16));
		write_cmos_sensor(GROUP_HOLD_ADDR, 0x01);
		mdelay(40);
		break;
	case MSDK_SCENARIO_ID_CUSTOM5:
		...
		table_write_cmos_sensor(agates5khm2_switch_fastmode_2x,
			sizeof(agates5khm2_switch_fastmode_2x) / sizeof(kal_uint16));
		write_cmos_sensor(GROUP_HOLD_ADDR, 0x01);
		S5KHM2_LOG_DBG("seamless switch to 4x mode!\n");
		mdelay(40);
		break;
	default:
		pr_info(
		...
		UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
	...
	case SENSOR_FEATURE_GET_VC_INFO:
		S5KHM2_LOG_DBG("SENSOR_FEATURE_GET_VC_INFO %d\n",
			(UINT16) *feature_data);
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		S5KHM2_LOG_DBG("odin GET_PDAF_DATA EEPROM\n");
		break;
	...
	case SENSOR_FEATURE_SET_PDAF:
		...
		S5KHM2_LOG_DBG("[odin] pdaf mode : %d \n", imgsensor.pdaf_mode);
		break;
#endif
...
UINT32 AGATES5KHM2_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    S5KHM2_LOG_DBG("AGATES5KHM2_MIPI_RAW_SensorInit in\n");
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}	/*	AGATES5KHM2_MIPI_RAW_SensorInit	*/
