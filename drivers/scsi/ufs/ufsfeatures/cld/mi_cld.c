				       ...
				       unsigned int val)
{
	...
	ufshcd_wait_for_doorbell_clr(hba, U64_MAX, false, 0, 0);
...
static void ufscld_trigger_work_fn(struct work_struct *dwork)
{
	...
	if (CLD_STATUS_IDLE == op_status) {//cld entry idel
...
void ufscld_init(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		memset(cld,0,sizeof(struct ufscld_dev));
