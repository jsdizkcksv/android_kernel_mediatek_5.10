...
#include "ufsffu.h"
#include "MICRON_128GB_0107.h"
#include "SKhynix_128G_A043.h"
#include "SKhynix_256G_A003.h"
#include "SAMSUNG_256GB_P07.h"
...
static struct fw_update_checklist need_update_k[] = {
	 ...
	 */
	{ "MICRON", "MT128GAXAU2U227X", "0101", "0107",
	  (uint64_t)MICRON_128GB_0107, sizeof(MICRON_128GB_0107) },
	{ "SKhynix", "H9HQ15AECMBDAR", "A002", "A043",
	  (uint64_t)SKhynix_128GB_A043, sizeof(SKhynix_128GB_A043) },
	{ "SKhynix", "H9QT1G6DN6X132", "A001", "A003",
	  (uint64_t)SKhynix_256GB_A003, sizeof(SKhynix_256GB_A003) },
	{ "SKhynix", "H9QT1G6DN6X132", "A002", "A003",
	  (uint64_t)SKhynix_256GB_A003, sizeof(SKhynix_256GB_A003) },
	{ "SAMSUNG", "KM8F9001JM-B813", "0100", "0700",
	  (uint64_t)SAMSUNG_256GB_P07, sizeof(SAMSUNG_256GB_P07) },
};
 ...
 */
static int ufs_scsi_write_partition(struct scsi_device *sdev, void *buf,
					uint32_t lba, uint32_t blocks)
{
	...
	ret = scsi_execute_req(sdev, cdb, DMA_TO_DEVICE, buf,
				(blocks * PART_BLOCK_SIZE), &sshdr,
				msecs_to_jiffies(15000), 3, NULL);
	if (ret)
 ...
 */
static int ufs_scsi_read_partition(struct scsi_device *sdev, void *buf,
				uint32_t lba, uint32_t blocks)
{
	uint8_t cdb[16];
	int ret = 0;
...
{
	if (strncmp((char *)stdinq->vendor_id, (char *)fw_info->vendor_id,
			strlen(fw_info->vendor_id))) {
		pr_notice("[ufs_ffu] check fw vendor_id not matching %s\n",
			(char *)fw_info->vendor_id);
		return 1;
	}
	if (strncmp((char *)stdinq->product_id, (char *)fw_info->product_id,
			strlen(fw_info->product_id))) {
		pr_notice("[ufs_ffu] check fw product_id not matching %s\n",
			(char *)fw_info->product_id);
		return 2;
	}
	if (!strncmp((char *)stdinq->product_rev,
			(char *)fw_info->product_rev_to,
			strlen(fw_info->product_rev_to))) {
...
static int get_fw_update_checklist(struct scsi_device *sdev,
				struct fw_update_checklist **need_update,
				struct ffu_inquiry *stdinq)
{
	...
	*need_update = kmalloc(list_count*sizeof(struct fw_update_checklist), GFP_KERNEL);
	...
	for (i = 0; i < part_head.ffu_count; i++) {
		...
		memcpy(&fw_info,(buf + FFU_BIN_HEAD_INFO + i * FFU_BIN_HEAD_INFO),
			sizeof(struct fw_update_checklist));
		crc = (unsigned int *)(buf + FFU_BIN_HEAD_INFO + i * FFU_BIN_HEAD_INFO +
			sizeof(struct fw_update_checklist));
		...
		size = fw_info.fw_size % PART_BLOCK_SIZE ? ((fw_info.fw_size / PART_BLOCK_SIZE) + 1) *
			PART_BLOCK_SIZE : fw_info.fw_size;
...
static int ufs_scsi_write_buf(struct scsi_device *sdev, uint8_t *buf,
				uint8_t mode, uint8_t buf_id, int32 offset, uint32 len)
{
	struct ufs_hba *hba = NULL;
	...
	ret = scsi_execute(sdev, cdb, DMA_TO_DEVICE, buf, len, NULL, &sshdr,
			 msecs_to_jiffies(15000), 0, 0, RQF_PM, NULL);
...
static int start_ffu(struct ffu_data *ffudata)
{
	...
	for (i = 0; i < fw_update_checklist_size; i++) {
		...
		if (strncmp((char *)stdinq.vendor_id,(char *)need_update[i].vendor_id,
				strlen(need_update[i].vendor_id)) == 0 && strncmp((char *)stdinq.product_id,
				(char *)need_update[i].product_id, strlen(need_update[i].product_id)) == 0) {
			pr_info("match vendor:%s, product id:%s hw_sectors %d\n",stdinq.vendor_id,
				 stdinq.product_id,queue_max_hw_sectors(ffudata->sdev->request_queue));

			/*step 2.  match product revision from */
			if (strncmp((char *)stdinq.product_rev,(char *)need_update[i].product_rev_from,
					 strlen(need_update[i].product_rev_from)) == 0) {
				pr_info("match product revision from:%s\n", stdinq.product_rev);
				pr_info("start write buffer\n");
				pr_info("fw_size = %d, address = %p\n",need_update[i].fw_size,
					need_update[i].fw_addr);
				ret = ufs_fw_update_write(ffudata->sdev, need_update[i].fw_size,
					(uint8_t *)need_update[i].fw_addr);
				if (ret == 0) {
					pr_notice("write buffer success\n");
					/* kernel_restart don't  work sometimes. */
					/* use kernel_restart and sleep 10ms to print above logs */
					msleep(20);
					ufs_ffu_reboot(NULL);
				} else {
			...
			} else if (strncmp((char *)stdinq.product_rev,
					(char *)need_update[i].product_rev_to,
					strlen(need_update[i].product_rev_to)) == 0) {
				/* step 3. match the product version to after reboot. */
				/* do not use loop just for reminder. */
			...
			pr_info("ufs_ffu do not match %d:%s %s\n", i, need_update[i].vendor_id,
				need_update[i].product_id);
		}
	}
...
static void ffu_ffu_delayed_work(struct work_struct *work)
{
	...
	struct ffu_data *ffudata = container_of(to_delayed_work(work),
					struct ffu_data, ffu_delayed_work);
