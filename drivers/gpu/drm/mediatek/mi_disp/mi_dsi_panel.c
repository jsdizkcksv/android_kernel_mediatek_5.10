			...
			int brightness)
{
	...
	struct mtk_ddp_comp *comp =  NULL;
	struct mtk_panel_ext *panel_ext = NULL;
#if defined(CONFIG_DRM_PANEL_K11T_42_02_0A_DSC_CMD)
	int hbm_brightness = 4095;
#endif
...
#if defined(CONFIG_DRM_PANEL_K11T_42_02_0A_DSC_CMD)
	if(dsi->mi_cfg.last_bl_level == 4091 && brightness == 2047)
		hbm_brightness = 4091;
	else
		hbm_brightness = brightness;
	pr_debug("%s DEFAULT_MAX_BRIGHTNESS = %d dsi->mi_cfg.last_bl_level = %d brightness = %d hbm_brightness = %d", __func__, DEFAULT_MAX_BRIGHTNESS, dsi->mi_cfg.last_bl_level, brightness, hbm_brightness);
#endif
...
#if defined(CONFIG_DRM_PANEL_K11T_42_02_0A_DSC_CMD)
		if (!panel_ext->funcs->setbacklight_control(dsi->panel, hbm_brightness)) {
#else
...
int mi_dsi_panel_set_disp_param(struct mtk_dsi *dsi, struct disp_feature_ctl *ctl)
{
	...
	case DISP_FEATURE_CRC:
		...
		case CRC_SRGB:
		{
			...
			panel_ext->funcs->panel_set_crc_srgb(dsi->panel);
#if defined(CONFIG_DRM_PANEL_K11T_42_02_0A_DSC_CMD)
			pr_info("CRC srgb unlock commit ");
			if (private)
				mutex_unlock(&private->commit.lock);
#endif
			break;
		...
		case CRC_P3_D65:
		{
			...
			panel_ext->funcs->panel_set_crc_p3_d65(dsi->panel);
#if defined(CONFIG_DRM_PANEL_K11T_42_02_0A_DSC_CMD)
			pr_info("CRC p3 d65 unlock commit");
			if (private)
				mutex_unlock(&private->commit.lock);
#endif
			break;
