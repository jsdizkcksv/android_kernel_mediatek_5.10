...
ssize_t mi_dsi_display_enable_calib(void *display, char *buf)
{
	...
	return mi_dsi_panel_enable_calib(dsi, buf);
}

ssize_t mi_dsi_display_get_calib_status(char *buf)
{
	return mi_dsi_panel_get_calib_status(buf);
}
			...
			char *buf, size_t size)
{
	...
	name = kzalloc(50 * sizeof(unsigned char), GFP_KERNEL);
	count = mi_dsi_display_read_panel_info(dsi_display, name);

	pr_info("read panel_name count:%d name:%s \n", count, name);
	if (strstr(name, "mdss_dsi_k11t_42_02_0a_dsc_cmd")) {
		kfree(name);
		return mi_panel_wpinfo_read(buf, size);
	}
...
ssize_t mi_dsi_display_set_disp_count(void *display, char* buf)
{
	...
	struct lcm *ctx = NULL;
	if (dsi_display != NULL) {
		ctx = panel_to_lcm(dsi_display->panel);
        }
