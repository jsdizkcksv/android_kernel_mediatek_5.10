				  ...
				  struct snd_soc_component *component)
{
	...
	if (!component->debugfs_root) {
		adsp_err(dsp, "No component->debugfs_root\n");
		goto err;
	}
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	if (dsp[e->shift_l].booted) {
		dev_err(dsp->dev, "error happen, booted = %d\n", dsp[e->shift_l].booted);
		ret = -EBUSY;
	}
	else if (dsp[e->shift_l].compr)  {
		dev_err(dsp->dev, "error happen, compr is NOT NULL\n");
		ret = -EBUSY;
	}
	else
...
int wm_adsp2_component_probe(struct wm_adsp *dsp, struct snd_soc_component *component)
{
	...
	dsp->component = component;
	snd_soc_add_component_controls(dsp->component,
					  wm_adsp_cal_controls,
					  ARRAY_SIZE(wm_adsp_cal_controls));

	return snd_soc_add_component_controls(dsp->component, &dsp->fw_ctrl, 1);
}
