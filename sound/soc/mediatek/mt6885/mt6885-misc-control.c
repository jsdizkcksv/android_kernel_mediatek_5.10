...
extern unsigned int elliptic_add_platform_controls(struct snd_soc_component *component);
#endif
...
static int mt6885_afe_vow_barge_in_get(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
	...
	id = MT6885_BARGE_IN_MEMIF;
	ucontrol->value.integer.value[0] = afe->memif[id].vow_barge_in_enable;

	return 0;
}

static int mt6885_afe_vow_barge_in_set(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct mtk_base_afe *afe = snd_soc_component_get_drvdata(cmpnt);
...
static const struct snd_kcontrol_new mt6885_afe_barge_in_controls[] = {
	SOC_SINGLE_EXT("Vow_bargein_echo_ref", SND_SOC_NOPM, 0, 0x1, 0,
		       mt6885_afe_vow_barge_in_get,
		       mt6885_afe_vow_barge_in_set),
};
